...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	file_b = read(file_a, header, sizeof(Elf64_Ehdr));
	if (file_b == -1)
	{
		free(header);
		close_elf(file_a);
	...
	check_elf(header->e);
	printf("ELF Header:\n");
	print_magic(header->e);
	print_class(header->e);
	print_data(header->e);
	print_version(header->e);
	print_osabi(header->e);
	print_abi(header->e);
	print_type(header->e_type, header->e);
